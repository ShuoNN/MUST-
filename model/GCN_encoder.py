...
class GCN_Two(nn.Module):
    def __init__(self):
        ...
        self.gc1 = GraphConvolution(nfeat, nhid) # 768 768
        self.gc2 = GraphConvolution(nhid, nout) # 768 768
...
class AST_Model(nn.Module):
    ...
    def forward(self, adj, A2, A3, A4, A5):
        ...
        # output5 = self.gcn5(output4, A5)
        # gcn_output = 0.1*output1 + 0.2*output2 + 0.7*output3
        AST_output = 0.1*adj + 0.2*A2 + 0.7*A3
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V):
        ...
        '''# 在下面做改动，将矩阵添加到下面的公式里
        scores = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(d_k)  # Q*K的转置，除以根号dk   scores : [batch_size, n_heads, len_q, len_k]
        # scores.masked_fill_(attn_mask, -1e9)  # Fills elements of self tensor with value where mask is True.

        attn = nn.Softmax(dim=-1)(scores) # 进行softmax然后乘以scores
        context = torch.matmul(attn, V)  # 最后乘上V [batch_size, n_heads, len_q, d_v]
        return context, attn
...
class GCNEncoder(nn.Module):
    def __init__(self):
        ...
        # 设定encoder的个数
        self.ast_output = AST_Model() # 五个GCN 变动
        #self.layers = nn.ModuleList([EncoderLayer() for _ in range(n_layers)]) # AST Encoder？ 变动

    def forward(self, x, a, a2, a3, a4, a5):
        '''
        enc_inputs: [batch_size, src_len]
        '''
        # ast_embed = self.ast_output(x, a, a2, a3, a4, a5)  # 变动 gcn-output，就是对整个AST处理过后得到的嵌入向量，astEncoder的输入
        ast_embed = self.ast_output(a, a2, a3, a4, a5)
        ...
        for layer in self.layers:
            ...
            ast_outputs, enc_self_attn = layer(ast_embed)  # 变动 ast encoder层
