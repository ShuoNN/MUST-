...
import math
import numpy as np
from multi_modal import Multi_model
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        context, attn = ScaledDotProductAttention()(Q, K, V, attn_mask) # 这里
...
class MultiHeadAttention1(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V):
        ...
        # context: [batch_size, n_heads, len_q, d_v], attn: [batch_size, n_heads, len_q, len_k]
        context, attn = ScaledDotProductAttention1()(Q, K, V)
        context = context.transpose(1, 2).reshape(batch_size, -1,
                                                  n_heads * d_v)  # context: [batch_size, len_q, n_heads * d_v]
        output = self.fc(context)  # [batch_size, len_q, d_model]
        # 残差和层归一化
...
class MultiDecoderLayer(nn.Module):
    def __init__(self):
        ...
        self.dec_multi_attn = MultiHeadAttention1()
        self.pos_ffn = PoswiseFeedForwardNet()
    ...
    def forward(self, dec_inputs, enc_outputs, multi_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        dec_outputs, dec_self_attn = self.dec_self_attn(dec_inputs, dec_inputs, dec_inputs, dec_self_attn_mask) #  Mask attention
        # dec_outputs: [batch_size, tgt_len, d_model], dec_enc_attn: [batch_size, h_heads, tgt_len, src_len]
        dec_outputs, dec_enc_attn = self.dec_enc_attn(dec_outputs, enc_outputs, enc_outputs, dec_enc_attn_mask) # code attention
        dec_outputs, dec_ast_attn = self.dec_multi_attn(dec_outputs, multi_outputs, multi_outputs) # acf attention si-SAN
...
class Encoder(nn.Module):
    def __init__(self, src_vocab_size):
        ...
        self.src_vocab_size = src_vocab_size # code  
...
class MultiDecoder(nn.Module):
    def __init__(self, tgt_vocab_size, max_ast_node, src_max_length):
        ...
        self.tgt_vocab_size = tgt_vocab_size  # nl 324*512
        self.tgt_emb = nn.Embedding(self.tgt_vocab_size, d_model)
        self.pos_emb = PositionalEncoding(d_model)
        self.layers = nn.ModuleList([MultiDecoderLayer() for _ in range(n_layers)]) # 三个Attention和FFN
        self.multi = Multi_model(tgt_vocab_size, max_ast_node, src_max_length) # acf
        # self.AST_embed = AST_Model()

    def forward(self, dec_inputs, enc_inputs, enc_outputs,ast_outputs):   # 变动
        '''
        dec_inputs: [batch_size, tgt_len]
        enc_intpus: [batch_size, src_len]
        enc_outputs: [batsh_size, src_len, d_model]
        '''
        # multi_model_out = self.multi(ast_outputs, enc_outputs, ast_embed)   # 变动 融合
        multi_model_out = self.multi(enc_outputs,ast_outputs)   # 变动 融合
...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, max_ast_node, src_max_length):
        super(Transformer, self).__init__()
        self.encoder = Encoder(src_vocab_size).to(device)
        self.decoder1 = MultiDecoder(tgt_vocab_size, max_ast_node, src_max_length).to(device)
        # self.decoder = Decoder(tgt_vocab_size).to(device)
        self.projection = nn.Linear(d_model, tgt_vocab_size, bias=False).to(device)

    def forward(self, enc_inputs, dec_inputs, ast_outputs):    # 变动
        '''
        enc_inputs: [batch_size, src_len]
        dec_inputs: [batch_size, tgt_len]
        '''
        ...
        # print()
        # dec_outputs2, dec_self_attns2, dec_enc_attns2 = self.decoder(dec_inputs, enc_inputs, enc_outputs)
        # dec_outpus: [batch_size, tgt_len, d_model], dec_self_attns: [n_layers, batch_size, n_heads, tgt_len, tgt_len], dec_enc_attn: [n_layers, batch_size, tgt_len, src_len]
        
        # dec_outputs1, dec_self_attns, dec_enc_attns, dec_ast_attns = self.decoder1(dec_inputs, enc_inputs, enc_outputs, ast_outputs, ast_embed) 
        dec_outputs1, dec_self_attns, dec_enc_attns, dec_ast_attns = self.decoder1(dec_inputs, enc_inputs, enc_outputs, ast_outputs)   # 变动
        dec_logits1 = self.projection(dec_outputs1)  # dec_logits: [batch_size, tgt_len, tgt_vocab_size]
        # dec_logits2 = self.projection(dec_outputs2)
